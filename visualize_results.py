...
def visualize_latency_breakdown(csv_path):
    ...
    for idx, quant_type in enumerate(quant_types):
        ...
        for i, impl in enumerate(implementations):
            ...
            # Add total time on top of each bar with modern styling
            ax.text(x_pos[i], bottom + 0.0025, f'{bottom:.3f}ms',
                   ha='center', va='bottom', fontsize=9, fontweight='bold',
                   bbox=dict(boxstyle='round,pad=0.3', facecolor='#F7CA18', alpha=0.7))
...
def visualize_performance(csv_path):
    ...
    # Convert to numeric
    for col in ['MXFP4_ms', 'NF4_ms', 'NVFP4_ms']:
    ...
    quant_types = ['MXFP4_ms', 'NF4_ms', 'NVFP4_ms']
    quant_labels = ['MXFP4', 'NF4', 'NVFP4']
    width = 0.18
...
def visualize_performance_with_full(csv_path):
    """Visualize performance results from CSV file"""
    ...
    for i, impl in enumerate(implementations):
        values = [perf_df.loc[i, col] for col in quant_types]
        x_pos = np.arange(len(quant_types)) + i * 0.17
        if impl == "Full":
            impl = "Full-Fused"
        label_text = f"{impl.replace('_', ' ')} Flash Attention" if impl != "Ours" else impl
        bars = ax.bar(x_pos, values, width, label=label_text, color=colors[i])
    ...
    # Save figure
    output_path = csv_path.replace('.csv', '_with_full.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Visualization saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    if "latency_breakdown" in csv_path:
        ...
        visualize_performance_with_full(csv_path)
